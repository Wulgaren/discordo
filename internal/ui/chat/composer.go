...
// of acting as the focus_composer shortcut. Mirrors the mention context check
// in tabComplete.
func (c *composer) mentionTabPending() bool {
	if c.chat.GetVisible(mentionsListLayerName) {
		return true
	}
	_, _, r := c.GetWordUnderCursor(isMentionChar)
	return r == '@'
}
...
func (c *composer) Update(msg tview.Msg) tview.Cmd {
	...
	case tview.KeyMsg:
		if msg.Key() == tcell.KeyUp && !c.chat.GetVisible(mentionsListLayerName) {
			if strings.TrimSpace(c.GetText()) == "" && len(c.sendMessageData.Files) == 0 {
				if cmd := c.chat.messagesList.editLastOwnMessage(); cmd != nil {
					return cmd
				}
			}
		}
